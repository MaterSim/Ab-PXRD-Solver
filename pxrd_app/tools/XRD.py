...
class XRD:
    ...
    def get_plot_gsas2(self, U=0.005, V=-0.005, W=0.02, X=0.01, Y=0.01,
                       bg_order=6, bg_ratio=0.05, mix_ratio=0.02, dx=0.02,
                       eta=0.5, orientation=0.1, thermo=0.1,
                       add_noise=False, noise_level=0.005, max_counts=10000,
                       random_seed=None, normalize_to_100=True, particle_size=20):
        """
        Generate a simulated XRD plot using Caglioti profile functions (GSAS-II style).
        Defaults are tuned for a lab Cu Kα (Bragg–Brentano) setup using degrees.

        Args:
            U (float): Caglioti U parameter (Gaussian broadening, tan²θ term, deg²). [Cu Kα default ~0.005]
            V (float): Caglioti V parameter (Gaussian broadening, tanθ term, deg²). [Cu Kα default ~−0.005]
            W (float): Caglioti W parameter (Gaussian broadening, constant term, deg²). [Cu Kα default ~0.02]
            X (float): Lorentzian X parameter (1/cosθ term, deg). [Cu Kα default ~0.01]
            Y (float): Lorentzian Y parameter (tanθ term, deg). [Cu Kα default ~0.01]
            bg_order (int): Order of the polynomial background.
            bg_ratio (float): Ratio of background intensity.
            mix_ratio (float): Ratio of random noise intensity.
            dx (float): Step size for the simulated XRD.
            eta (float): Mixing parameter for pseudo-Voigt (0=pure Gaussian, 1=pure Lorentzian).
            orientation (float): Preferred orientation factor.
            thermo (float): Thermal vibration factor (Debye-Waller).
            add_noise (bool): If True, add Poisson + Gaussian noise to intensities.
            noise_level (float): Relative Gaussian noise level (e.g., 0.01 = 1%).
            max_counts (int): Counts scaling used before applying Poisson noise.
            random_seed (int|None): Optional RNG seed for reproducibility.
            normalize_to_100 (bool): If True, scale output to [0,100]. If False, keep in counts/relative units.
            particle_size (float|None): Particle size in nanometers for finite-size effect convolution.
                                       If None, no finite-size effect is applied.
        ...
        # Marked locations and intensities
        x, y = self.pxrd[:, 0], self.pxrd[:, -1] * 100
        thetas = np.radians(x / 2)  # Convert 2theta to theta

        # Caglioti function for Gaussian FWHM²
        # FWHM_G² = U*tan²θ + V*tanθ + W
        tan_theta = np.tan(thetas)
        fwhm_g_sq = U * tan_theta**2 + V * tan_theta + W
        fwhm_g_sq = np.maximum(fwhm_g_sq, 0.01)  # Ensure non-negative
        fwhm_g = np.sqrt(fwhm_g_sq)  # Gaussian FWHM in degrees

        # Lorentzian FWHM
        # FWHM_L = X/cosθ + Y*tanθ
        cos_theta = np.cos(thetas)
        fwhm_l = X / (cos_theta + 1e-10) + Y * tan_theta
        fwhm_l = np.maximum(fwhm_l, 0.01)  # Ensure positive

        # Convert FWHM to radians for calculation
        fwhm_g_rad = np.radians(fwhm_g)
        fwhm_l_rad = np.radians(fwhm_l)

        # For pseudo-Voigt, we need Gaussian and Lorentzian HWHM
        # Gaussian: HWHM = FWHM / (2*sqrt(2*ln2))
        # Lorentzian: HWHM = FWHM / 2
        hwhm_g = fwhm_g_rad / (2 * np.sqrt(2 * np.log(2)))
        hwhm_l = fwhm_l_rad / 2

        # Convert to Gaussian variance and Lorentzian gamma for Voigt
        sigma2 = hwhm_g ** 2  # Gaussian variance
        gamma = hwhm_l  # Lorentzian HWHM
        ...
        # Add each peak contribution using pseudo-Voigt profile
        for k in range(len(x)):
            if x[k] < theta_max:
                # Use pseudo-Voigt profile directly (simpler than full Voigt convolution)
                y_sim += add_peak_caglioti(x_sim, x[k], gamma[k], sigma2[k], eta, dx) * y[k]
        ...
        # Apply finite-size effect if particle_size is specified
        if particle_size is not None:
            y_sim = apply_finite_size_effect(x_sim, y_sim, particle_size, self.wavelength)
        ...
        if add_noise and y_sim.max() > 0:
            if random_seed is not None:
                np.random.seed(int(random_seed))
            counts = y_sim / y_sim.max() * max_counts
            ...
            y_sim = noisy_counts * (y_sim.max() / max_counts)

        # Optional normalization to (0, 100)
        if normalize_to_100:
            y_min, y_max = y_sim.min(), y_sim.max()
            if y_max > y_min:
                y_sim = (y_sim - y_min) / (y_max - y_min) * 100
...
def apply_finite_size_effect(twotheta, intensity, particle_size, wavelength):
    """
    Apply finite-size effect to XRD pattern via convolution with sinc² function.

    Following the approach: I_nano = I_c ⊗ sinc²(Qτ/2)
    where τ is the particle size and Q is the scattering vector magnitude.

    Args:
        twotheta (array-like): Array of 2-theta values in degrees.
        intensity (array-like): Array of intensity values.
        particle_size (float): Particle size (τ) in nanometers.
        wavelength (float): X-ray wavelength in Angstroms.

    Returns:
        ndarray: Intensity array after applying finite-size effect.
    """
    # Convert particle size from nm to Angstroms
    tau = particle_size * 10.0

    # Calculate Q = 4π sin(θ) / λ for each 2θ
    theta_rad = np.radians(twotheta / 2)
    Q = 4 * np.pi * np.sin(theta_rad) / wavelength

    # Create the sinc² kernel: sinc²(Qτ/2)
    # The kernel needs to be in 2θ space, so we evaluate sinc² at each Q value
    # For convolution, we need to create a kernel that spans the relevant Q range

    # Determine kernel width in 2θ space
    # The first zero of sinc(x) occurs at x = π, so Qτ/2 = π → Q = 2π/τ
    # This gives us a characteristic width in Q space
    dQ = 2 * np.pi / tau

    # Convert this to 2θ space at the median angle (for approximation)
    median_theta = np.radians(np.median(twotheta) / 2)
    median_Q = 4 * np.pi * np.sin(median_theta) / wavelength

    # Estimate d(2θ)/dQ at median point
    # Q = 4π sin(θ) / λ, so dQ/dθ = 4π cos(θ) / λ
    # and dQ/d(2θ) = 2π cos(θ) / λ
    d2theta_dQ = wavelength / (2 * np.pi * np.cos(median_theta))

    # Kernel width in 2θ space (extend to ~3 zeros of sinc for good coverage)
    kernel_width_2theta = 6 * np.pi * d2theta_dQ / tau

    # Create kernel in 2θ space
    d2theta = twotheta[1] - twotheta[0]  # Assume uniform spacing
    n_kernel = int(kernel_width_2theta / d2theta)
    if n_kernel % 2 == 0:
        n_kernel += 1  # Make it odd for symmetry

    kernel_2theta = np.linspace(-kernel_width_2theta/2, kernel_width_2theta/2, n_kernel)
    kernel_theta_rad = np.radians((np.median(twotheta) + kernel_2theta) / 2)
    kernel_Q = 4 * np.pi * np.sin(kernel_theta_rad) / wavelength

    # Compute sinc²(Qτ/2) kernel
    x_kernel = kernel_Q * tau / 2
    # Handle sinc at origin
    sinc_kernel = np.ones_like(x_kernel)
    mask = x_kernel != 0
    sinc_kernel[mask] = np.sin(x_kernel[mask]) / x_kernel[mask]
    kernel = sinc_kernel ** 2

    # Normalize kernel
    kernel /= np.sum(kernel)

    # Apply convolution
    intensity_conv = np.convolve(intensity, kernel, mode='same')

    return intensity_conv




def add_peak_caglioti(twotheta, mu, gamma, sigma2, eta=0.5, step=0.02):
    """
    Add a single peak using pseudo-Voigt profile (GSAS-II Caglioti style).
    This is a simpler profile function that matches GSAS-II's approach.

    Args:
        twotheta (array-like): Array of 2-theta values.
        mu (float): Peak center (2-theta) in degrees.
        gamma (float): Lorentzian HWHM parameter (radians).
        sigma2 (float): Gaussian variance parameter (radians²).
        eta (float): Mixing parameter (0=Gaussian, 1=Lorentzian).
        step (float): Step size for the 2-theta array.

    Returns:
        ndarray: Array of same shape as twotheta with peak intensity.
    """
    ...
    result = np.zeros_like(twotheta)
    result[mask] = pseudo_voigt[mask]

    # Normalize to unit area
    area = np.trapz(result, twotheta)
    if area > 0:
        result /= area
...
class Profile:
    ...
    def get_profile(self, two_thetas, intensities, min2theta, max2theta):
        ...
        Args:
            - two_thetas: 1d float array simulated 2 theta values
            - intensities: simulated peaks
        """
